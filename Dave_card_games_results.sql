...
SELECT MAX(score) as max_score, MIN(score) as min_score FROM card_games;
...
-- Games with an average score above a certain threshold (e.g., 200):
...
/*Grouping Data Values That Don't Exist Yet (Using CASE):*/

-- Grouping scores into categories (e.g., low, medium, high):

SELECT
  CASE
    WHEN score < 100 THEN 'Low'
    WHEN score >= 100 AND score < 200 THEN 'Medium'
...
-- Players who played Rummy or Crazy Eights:

SELECT DISTINCT player_name
FROM card_games
WHERE game_name = 'Rummy' OR game_name = 'Crazy Eights';

-- Games played by Spunky Sam with a score above 300:
SELECT game_name, score
FROM card_games
WHERE player_name = 'Spunky Sam' AND score > 300;
